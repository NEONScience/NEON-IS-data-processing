...
def.loc.meta <- function(NameFile,
  ...
  # Get site level properties including location properties from location histories
  if (!is.null(nameProp)) {
    return(get.location.properties(locPropMore, nameProp))
  }
...
get.location.properties <- function (data, nameProp) {
  # data is a list with items '$locations$features[[1]]$properties$location_properties'
  # Access the "location_properties" list
  location_properties <- data[[1]]$locations$features[[1]]$properties$location_properties
  
  # extract the properties as a named list
  location_properties_list <- sapply(location_properties, function(x) setNames(x[[2]], x[[1]]))
  # Now you can access properties using their names

  properties <- base::lapply(nameProp,FUN=function(idxNameProp){
    if(idxNameProp %in% names(location_properties_list)) {
      if(base::is.null(location_properties_list[[idxNameProp]])){
        return(NA)
      } else {
        return(location_properties_list[[idxNameProp]])
      }
    } else {
      return(data[[1]][[idxNameProp]])
    }
  })
  base::names(properties) <- nameProp
  return(properties)
}