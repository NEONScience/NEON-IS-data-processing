...
##############################################################################################
filter_threshold <- function(DirIn,
                             DirOut,
                             Term = NULL,
                             Ctxt = NULL) {
  # Start logging
  ...
  log$info(
    base::paste0(
      'Filtering thresholds for Term = ',
      base::paste0(Term, collapse = ','),
      ', Context = ',
      base::paste0(Ctxt, collapse = ',')
    )
  )
  ...
  if (base::length(file) > 1) {
    log$warn(
      base::paste0(
        'There are is more than one threshold file in path: ',
        DirIn,
        '... Filtering them all!'
      )
    )
  ...
  for (idxFile in file) {
    ...
    NEONprocIS.qaqc::def.thsh.json.filt(
      NameFile = fileIn,
      NameFileOut = fileOut,
      Term = Term,
      Ctxt = Ctxt
