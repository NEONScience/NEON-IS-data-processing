###############################################################################
#' @title Convert Temperature at Specific Depth in Lakes from wide to long
...
Para <- base::list(DirIn = "~/pfs/tempSpecificDepthLakes_level1_group/tchain/2019/01/10/",#CFGLOC110702/",
                   DirOut = "~/pfs/tsdl_comb_long/",
                   NameDirCombOut = "tsdlCombLongOut",
...
# NameFileSufx="_basicStats_001" in tempSpecificDepthLakes_stats_instantaneous.yaml 
...
log$debug(
  base: # NameFileSufx="_basicStats_001" in tempSpecificDepthLakes_stats_instantaneous.yaml :paste0(
...
for (idxDirIn in DirIn) {
  ...
  dataLs <- # NEONprocIS.base::
   def.file.comb.tsdl.long(file = filePath,
  ...
  for(nameVarTime in Para$NameVarTime){
    ...
    # ----------------------------------------------------------------------- #
    # Remove suffix strings, Take the shortest file name
    # ----------------------------------------------------------------------- #
    
    # Subset to dat files that should begin with 'tchain'
    fileDat <- file[base::intersect(base::grep("tchain", file), base::grep(nameVarTime,file))] 
    
    # Remove the NameFileSufx strings to simplify output filename
    fileDats <-  base::lapply(Para$NameFileSufxRm, function(x) base::gsub("__","_",base::gsub(pattern = x, replacement = "", fileDat) ) )
    fileDats <- base::unlist(base::lapply(fileDats, function(x) x[base::which.min(base::nchar(x))]))
