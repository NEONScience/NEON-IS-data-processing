...
calibration_conversion<- function(DirIn,
  ...
  log$debug(base::paste0(
    ...
    base::paste0(TermConv, collapse = ',')
  ))
  numConv <- base::length(TermConv)
  log$debug(base::paste0(
    'Terms to output calibration flags: ',
    base::paste0(TermQf, collapse = ',')
  ))
  
  # Read in the schemas so we only have to do it once and not every
  # time in the avro writer.
  if (!base::is.null(FileSchmData)) {
    # Retrieve and interpret the output data schema
    SchmDataOutAll <-
      NEONprocIS.base::def.schm.avro.pars(FileSchm = FileSchmData, log =
                                            log)
  ...
  log$debug(base::paste0(
    ...
    base::paste0(FuncConv, collapse = ',')
  ))
  if (!base::is.null(TermConv) && base::length(TermConv) > 0) {
    FuncConv <-
  ...
  log$debug(
    base::paste0(
      ...
      base::paste0(NumDayExpiMax, collapse = ',')
    )
  )
  
  # Parse parameters for individual measurement uncertainty
  if (!base::is.null(TermUcrt) && base::length(TermUcrt) > 0) {
    ...
    if (base::any(!base::is.na(ParaUcrt$typeFdas))){
      if (base::is.null(FileUcrtFdas)) {
        log$fatal('Path to FDAS uncertainty file must be input in argument FileUcrtFdas.')
        stop()
        
      } else {
        ucrtCoefFdas  <-
          NEONprocIS.cal::def.read.ucrt.coef.fdas(NameFile = FileUcrtFdas, 
                                                  log = log)
  ...
  for (idxDirIn in DirIn) {
    ...
    calSlct <- NEONprocIS.cal::wrap.cal.slct(
      DirCal = idxDirCal,
      NameVarExpc=base::unique(c(TermConv,TermQf,ParaUcrt$var)),
      TimeBgn = timeBgn,
      TimeEnd = timeEnd,
