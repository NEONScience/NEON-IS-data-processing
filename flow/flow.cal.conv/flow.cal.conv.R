...
filter_calibration_conversion <- function(DirIn, DirOut, FileSchmData, FileSchmQf, Term, NumDayExpiMax, TermUcrtFdas, FileUcrtFdas, DirSubCopy) {
  ...
  FileSchmData <- FileSchmData
  log$debug(base::paste0('Schema for output data: ',FileSchmData))
  
  FileSchmQf <- FileSchmQf
  log$debug(base::paste0('Schema for output flags: ',FileSchmQf))
  
  # Turn schema names to NULL if they are NA. This will cause the avro writer to create the schema 
  ...
  if(!base::is.null(TermUcrtFdas)){
    ParaFdas <- base::lapply(TermUcrtFdas,FUN=function(argSplt){
    ...
    ucrtFdas  <- base::try(rjson::fromJSON(file=FileUcrtFdas,simplify=TRUE),silent=FALSE)
  ...
  log$debug(base::paste0('Terms for which to compute FDAS uncertainty information: ',base::paste0(TermUcrtFdas,collapse=',')))
  ...
  for(idxDirIn in DirIn){
    ...
    exstCal <- Term %in% varCal
    if (base::sum(!exstCal > 0)){
      log$error(base::paste0('No calibration folder exists for term(s): ', base::paste0(Term[!exstCal],collapse=','), ' in datum path ',idxDirCal))
      stop()
    }
