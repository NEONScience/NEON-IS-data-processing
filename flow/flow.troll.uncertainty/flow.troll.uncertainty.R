##############################################################################################
#' @title Workflow for Level Troll 500 and Aqua Troll 200 additional Uncertainty Computations

#' @author
#' Nora Catolico \email{ncatolico@battelleecology.org}

#' @description Workflow. Derive uncertainty  for surface and groundwater troll data products.
#' 
#'
#' The arguments are: 
#' 
...
for (idxDirIn in DirIn){
  ...
  if(length(uncertaintyCoef)>0){
    ...
    uncertaintyData$temperature_ucrtExpn_L1<-NEONprocIS.stat::wrap.ucrt.dp01.cal.mult(data=trollData,VarUcrt='temperature',ucrtCoef=uncertaintyCoef)
    ...
    uncertaintyData$pressure_ucrtExpn_L1<-NEONprocIS.stat::wrap.ucrt.dp01.cal.mult(data=trollData,VarUcrt='pressure',ucrtCoef=uncertaintyCoef)
    uncertaintyData$pressure_ucrtComb_L1<-uncertaintyData$pressure_ucrtExpn_L1/2
    ...
    uncertaintyData$elevation_ucrtExpn_L1<-2*((1*trollData$survey_uncert^2+((1000/(density*gravity))^2)*uncertaintyData$pressure_ucrtComb_L1^2)^0.5)
    ...
    if(sensor=='aquatroll200'){
      ...
      uncertaintyData$rawConductivity_ucrtExpn_L1<-NEONprocIS.stat::wrap.ucrt.dp01.cal.mult(data=trollData,VarUcrt='conductivity',ucrtCoef=uncertaintyCoef)
      ...
      uncertaintyData$conductivity_ucrtComb_L1<-NA
      ...
      for(i in 1:length(uncertaintyData$conductivity_ucrtMeas)){
        uncertaintyData$conductivity_ucrtComb_L1[i]<-((se^2)*(((1/(1+0.0191*(trollData$temperature[i]-25)))^2)*U_CVALA3_cond^2)+((((0.0191*trollData$raw_conductivity[i])/((1+0.0191*(trollData$temperature[i]-25))^2))^2)*U_CVALA3_temp^2))^0.5
      }
      # Compute expanded uncertainty for L1 specific conductivity
      uncertaintyData$conductivity_ucrtExpn_L1<-2*uncertaintyData$conductivity_ucrtComb_L1
    }
  }else{
    uncertaintyData$temperature_ucrtExpn_L1<-NA
    uncertaintyData$pressure_ucrtExpn_L1<-NA
    uncertaintyData$elevation_ucrtExpn_L1<-NA
    uncertaintyData$conductivity_ucrtExpn_L1<-NA
  }
  ...
  if(context=="GW"){
    #5-min groudnwater data gets instantaneous uncertainties
    ucrtCol_05min <- c("readout_time","temperature_ucrtExpn","pressure_ucrtExpn","elevation_ucrtExpn","conductivity_ucrtExpn")
    #30-min groudnwater data gets mean uncertainties
    ucrtCol_30min <- c("readout_time","temperature_ucrtExpn_L1","pressure_ucrtExpn_L1","elevation_ucrtExpn_L1","conductivity_ucrtExpn_L1")
  }else if(sensor=="aquatroll200"){
    #5 and 30-min surface water data gets mean uncertainties. Conductivity included for aqua troll.
    ucrtCol_05min <- c("readout_time","temperature_ucrtExpn_L1","pressure_ucrtExpn_L1","elevation_ucrtExpn_L1","conductivity_ucrtExpn_L1")
    ucrtCol_30min <- c("readout_time","temperature_ucrtExpn_L1","pressure_ucrtExpn_L1","elevation_ucrtExpn_L1","conductivity_ucrtExpn_L1")
  }else{
    #5 and 30-min surface water data gets mean uncertainties. Conductivity not included for level troll.
    ucrtCol_05min <- c("readout_time","temperature_ucrtExpn_L1","pressure_ucrtExpn_L1","elevation_ucrtExpn_L1")
    ucrtCol_30min <- c("readout_time","temperature_ucrtExpn_L1","pressure_ucrtExpn_L1","elevation_ucrtExpn_L1")
  }
  ...
  if(context=="GW"){
    names(ucrtOut_05min)<- c("readout_time","groundwaterTempExpUncert","groundwaterPressureExpUncert","groundwaterElevExpUncert","groundwaterCondExpUncert")
    names(ucrtOut_30min)<- c("readout_time","groundwaterTempExpUncert","groundwaterPressureExpUncert","groundwaterElevExpUncert","groundwaterCondExpUncert")
  }else if(sensor=="aquatroll200"){
    names(ucrtOut_05min)<- c("readout_time","surfacewaterTempExpUncert","surfacewaterPressureExpUncert","surfacewaterElevExpUncert","surfacewaterCondExpUncert")
    names(ucrtOut_30min)<- c("readout_time","surfacewaterTempExpUncert","surfacewaterPressureExpUncert","surfacewaterElevExpUncert","surfacewaterCondExpUncert")
  }else{
    names(ucrtOut_05min)<- c("readout_time","surfacewaterTempExpUncert","surfacewaterPressureExpUncert","surfacewaterElevExpUncert")
    names(ucrtOut_30min)<- c("readout_time","surfacewaterTempExpUncert","surfacewaterPressureExpUncert","surfacewaterElevExpUncert")
  }
