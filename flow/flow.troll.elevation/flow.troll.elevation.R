...
#' @description Workflow. Calculate elevation for surface and groundwater troll data products.
#' 
#'
#' The arguments are: 
#' 
...
#' Data will be output in Parquet format in DirOut, where the terminal directory 
...
#' readout_time, pressure, temperature, elevation, in that order. 
#' Output column/variable names for the aquatroll200 will be readout_time, pressure, temperature, conductivity, elevation, in that order.
...
for (idxDirIn in DirIn){
  ...
  # Uncertainty for L1 mean 5 and 30 minute outputs will be done in/after stats module
  if(context=="GW"){
  ...
  #Create dataframe for instantaneous uncertainty
  ucrtOut <- uncertaintyData
  if(sensor=="aquatroll200"){
    ucrtCol <- c("readout_time","temperature_ucrtExpn","pressure_ucrtExpn","elevation_ucrtExpn","conductivity_ucrtExpn")
  }else{
    ucrtCol <- c("readout_time","temperature_ucrtExpn","pressure_ucrtExpn","elevation_ucrtExpn")
  }
  ucrtOut <- uncertaintyData[,ucrtCol]
  
  #standardize column names
  if(context=="GW"){
    names(ucrtOut)<- c("readout_time","groundwaterTempExpUncert","groundwaterPressureExpUncert","groundwaterElevExpUncert","groundwaterCondExpUncert")
  }else if(sensor=="aquatroll200"){
    names(ucrtOut)<- c("readout_time","surfacewaterTempExpUncert","surfacewaterPressureExpUncert","surfacewaterElevExpUncert","surfacewaterCondExpUncert")
  }else{
    names(ucrtOut)<- c("readout_time","surfacewaterTempExpUncert","surfacewaterPressureExpUncert","surfacewaterElevExpUncert")
  }
