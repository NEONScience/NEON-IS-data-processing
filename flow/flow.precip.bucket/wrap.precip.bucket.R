...
wrap.precip.bucket <- function(DirIn,
  ...
  # Initial processing to calculate combinedUcrt and its type
  if (grepl(loc$context, pattern = "throughfall")) {
    ...
    # Flag for quadrature (RSS) aggregation
    data$uncertainty_type <- "quadrature"
    ...
    # Flag for simple sum aggregation
    data$uncertainty_type <- "simple_sum"
  }
    ...
    dplyr::summarise(
      ...
      precipBulkExpUncert = {
        if (all(uncertainty_type == "quadrature")) {
          base::sqrt(base::sum(combinedUcrt^2)) * 2
        } else if (all(uncertainty_type == "simple_sum")) {
          base::sum(combinedUcrt) * 2 # Apply coverage factor here
        } else {
          # Handle mixed types or other cases if necessary (e.g., NA or error)
          NA_real_
        }
      },
      ...
      # Also carry the uncertainty_type forward to the aggregated data
      uncertainty_type = dplyr::first(uncertainty_type) # Assuming type is consistent within a group
    ) %>%
    ...
    dplyr::summarise(
      ...
      precipBulkExpUncert = {
        if (all(uncertainty_type == "quadrature")) {
          ...
          base::sqrt(base::sum((precipBulkExpUncert / 2)^2)) * 2
        } else if (all(uncertainty_type == "simple_sum")) {
          base::sum(precipBulkExpUncert, na.rm = T) # Summing the already K=2'd 1-min uncertainties
        } else {
          NA_real_
        }
      },
