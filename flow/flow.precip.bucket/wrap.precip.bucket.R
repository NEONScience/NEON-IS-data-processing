...
wrap.precip.bucket<- function(DirIn,
  ...
  dirInUcrt <- fs::path(DirIn, 'uncertainty_data')
  ...
  fileUcrt <- base::list.files(dirInQf,pattern='uncertaintyData.parquet',full.names=FALSE)
  ...
  #################TODO what happens if logical zero here? 
  ...
  qfCal<- NEONprocIS.base::def.read.parq.ds(fileIn=fs::path(dirInQf,fileQfCal),
                                                 ...
                                                 Df=TRUE,
                                                 log=log)
  ucrt <- NEONprocIS.base::def.read.parq.ds(fileIn=fs::path(dirInUcrt,fileUcrt),
                                                      VarTime='readout_time',
                                                      RmvDupl=TRUE,
                                                      Df=TRUE,
                                                      log=log)
  ...
  data <- full_join(data,ucrt, by = 'readout_time')
  ...
  #if throughfall, apply area downscaling of tip measurements. 
  
  # convert throughfall values (based on context)
  tf_conv <- 32429/251400
  ...
  stats_aggr30 <- stats_aggr01 %>% 
    ...
    dplyr::summarise(
      ...
      finalQF = as.integer(max(finalQF, na.rm = T)))  #make integer
