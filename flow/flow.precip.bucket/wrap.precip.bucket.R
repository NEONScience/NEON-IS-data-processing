...
#     Initial creation
# Teresa Burlingame (2025-07-11)
#     Adding in logic to handle sec and TF UCRT calcs differently. Reconfigured with Claude AI optimization
##############################################################################################
wrap.precip.bucket <- function(DirIn,
  ...
  dir_paths <- fs::path(DirIn, c('data', 'flags', 'threshold', 'location', 'uncertainty_coef'))
  names(dir_paths) <- c('data', 'flags', 'threshold', 'location', 'uncertainty_coef')
