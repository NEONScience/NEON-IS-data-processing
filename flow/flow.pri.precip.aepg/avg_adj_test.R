...
source(paste0(func_pat, 'avg_adj_primary_noise_func.R'))
...
#total pcp
df_hr$bench[nrow(df_hr)]-df_hr$bench[1]
df_five$bench[nrow(df_five)]-df_five$bench[1]

p <- ggplot(df_hr, aes(x=summ)) +
  geom_line(data = df_five, aes( y = bench,x = summ, color = 'Five Min'))+
