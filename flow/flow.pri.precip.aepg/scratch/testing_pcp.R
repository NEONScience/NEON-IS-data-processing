...
pcp_daily <- all_pcp %>% 
  ...
  group_by(day) %>% 
  summarise(bench = sum(bench_rt),
            adj_bench = sum(adj_bench_rt),
            sec_daily = sum(hourly),
            raw_daily = sum(raw_rt)) %>%
  mutate(tot_bench = cumsum(bench),
         tot_adj_bench = cumsum(adj_bench),
         tot_sec  = cumsum(sec_daily),
         tot_raw = cumsum(raw_daily)) 

prism$day <- as.POSIXct(as.character(prism$date),format = '%Y%m%d')
