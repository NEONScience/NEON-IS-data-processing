...
noise_removal = function(
  ...
  #### TB added evaporation handling
    ...
    if ((precip & !any(raw_df_summ$evap[i:currRow-1]) & (prev_bench - evapAdj > envelope*0.4))){ #was evap accounted for in last 24h?
      ...
      print(raw_df_summ$summ[currRow])
      print(paste0('evap not accounted, adjust by rain only: ',  bench - prev_bench))
