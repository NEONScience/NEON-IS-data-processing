...
#   Teresa Burlingame (2026-03-02)
#     Better handling of empty data frames. 
##############################################################################################
def.rad.shadow.flags <- function(DirIn, 
  ...
  if (length(fileLoc) == 0) {
    log$warn(base::paste0('No location data in ', dirInLoc))
    flagDf$shadowQF <- -1
    return(flagDf)
  }
  ...
  if (any(grepl(fileLoc, pattern = "locations.json"))) {
    ...
    log$warn(base::paste0('No geolocation data in path ', dirInLoc, ' cannot calculate shadows without lat/long setting flag to -1'))
    flagDf$shadowQF <- -1
    return(flagDf)
  }
  ...
  for(src in shadow_sources) {
    ...
    # Only merge and flag if there are shading events
    if (nrow(dt_shadow) > 0) {
