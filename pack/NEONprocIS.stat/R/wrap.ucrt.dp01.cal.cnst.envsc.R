...
#' due to natural variation and calibration (constant value). Specific naming convention changed
#' for enviroscan sensor to get soilSpecific calibration constant coefficient.
...
#' coefficient provided by CVAL (U_CVALA3_soilSpec).
...
#' ucrt <- NEONprocIS.stat::wrap.ucrt.dp01.cal.cnst.envsc(data=data,VarUcrt='VSWCsoilSpecificMean',ucrtCoef=ucrtCoef)
...
#     Branched off C.Sturtevant's wrap.ucrt.dp01.cal.cnst to create a constant-value wrapper specific to enviroscan soil-specific calibration
##############################################################################################
wrap.ucrt.dp01.cal.cnst.envsc <- function(data,
                                    VarUcrt,
                                    ucrtCoef,
                                    ucrtData=NULL,
                                    log = NULL) {
  ...
  # Compute calibration uncertainty (constant value from CVAL coefficient U_CVALA3_soilSpec)
  ucrtCal <- NEONprocIS.stat::def.ucrt.dp01.cal.cnst(ucrtCoef=ucrtCoef,
                                                     NameCoef='U_CVALA3_soilSpec',
                                                     VarUcrt=VarUcrt,
