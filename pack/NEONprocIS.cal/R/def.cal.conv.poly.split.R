...
#'                )
#' dataCal <- def.cal.conv.poly.split(data=data,varConv=c('var1','var2'),calSlct=calSlct)
...
def.cal.conv.poly.split <- function(data = data.frame(data=base::numeric(0)),
  ...
  for(varIdx in varConv){
    ...
    for(idxRow in base::seq_len(base::nrow(calSlctIdx))){
      ...
      setFunc01 <- setCal & !base::is.na(dataConvIdx) & (dataConvIdx > 100)
      dataConvOutIdx[setFunc01] <- stats::predict(object = func1, newdata = dataConvIdx[setFunc01])
      setFunc02 <- setCal & !base::is.na(dataConvIdx) & dataConvIdx <= 100
      dataConvOutIdx[setFunc02] <- stats::predict(object = func2, newdata = dataConvIdx[setFunc02])
