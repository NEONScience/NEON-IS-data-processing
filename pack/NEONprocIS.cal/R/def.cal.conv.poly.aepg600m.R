##############################################################################################
#' @title Convert raw to calibrated data using NEON CVALA polynomial calibration coefficients with origin F0 for 
...
#' @description
#' Definition function. Apply NEON calibration polynomial function with calibration coefficients and origin F0 for 
#' Belfort primary precip sensor strain gauges
...
def.cal.conv.poly.aepg600m <- function(data = data.frame(data=base::numeric(0)),
  ...
  for(varIdx in varConv){
    ...
    for(idxRow in base::seq_len(base::nrow(calSlctIdx))){
      ...
      dpthBckt <- stats::predict(object = func, newdata = dataConvIdx[setCal] - as.numeric(infoCal$cal[grep("P0", infoCal$cal$Name),"Value"]))*10 # Total bucket depth (mm)
      ...
      dataConvOutIdx[setCal] <- dpthBckt - dpthZero
