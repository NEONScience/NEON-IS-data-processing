##############################################################################################
#' @title Convert raw to calibrated data using NEON CVALL polynomial calibration coefficients
...
#' CVALL0, CVALL1, CVALL2, etc. to convert raw data to calibrated data. 
...
#' \code{Name} String. The name of the coefficient. Must fit regular expression CVALL[0-9]\cr
...
#' infoCal <- data.frame(Name=c('CVALL1','CVALL0'),Value=c(10,1),stringsAsFactors=FALSE)
...
def.cal.conv.poly.b <- function(data = data.frame(data=base::numeric(0)),
  ...
  func <-
    NEONprocIS.cal::def.cal.func.poly(infoCal = infoCal, Prfx='CVALL', log = log)
