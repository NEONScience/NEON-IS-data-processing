...
def.cal.conv <- function(data,
  ...
  levlPoly <- base::as.numeric(base::unlist(base::lapply(base::strsplit(cal$Name,'CVALA',fixed=TRUE),
                                                        FUN=function(vect){vect[2]})))
  ...
  dervCal <- stats::predict(object=funcDerv,newdata=data) # Eval derivative at each measurement
  
  # Compute uncertainty
  ucrt <- base::data.frame(dervCal=dervCal,ucrtMeas=coefUcrtMeas,ucrtFdas=NA*data,ucrtComb=NA*data)
  if(base::length(coefUcrtFdas)==1 && base::length(coefUcrtFdasOfst)==1){
    ucrt$ucrtFdas <- (coefUcrtFdas*data + coefUcrtFdasOfst)*base::abs(dervCal)
  }
