...
test_that("Unit test of def.ucrt.fdas.volt.poly.R", {
  ...
  uncertainty <-
    NEONprocIS.cal::def.ucrt.fdas.volt.poly(data = data, varUcrt='data', calSlct=calSlct)
  ...
  testthat::expect_equal(c(0.0123, 0.1000), 
                         ...
                         tolerance = 1E-6)
  testthat::expect_equal(c(0.6150615, 60.0010000), 
  ...
  uncertainty <- NEONprocIS.cal::def.ucrt.fdas.volt.poly(data = data, varUcrt='data', calSlct=calSlct)
