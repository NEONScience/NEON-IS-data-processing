...
test_that("testing calibration polynomial function", {
  ...
  ##########
  ########## Sad path #1 - infoCal is a list but has a wrong name, cal1, instead of cal 
  ##########
  ...
  func <-
    try(NEONprocIS.cal::def.cal.func.poly(infoCal = infoCal,
                                          log = log,
                                          Vrbs = TRUE),
        silent = TRUE)
  #
  ...
  ##########
  ########## Sad path #2 - when a data frame is passed to infoCal
  ##########
  Name = c("CVALA1", "CVALA2", "CVALA3", "CVALA4", "CVALA5", "CVALA6")
  Value = c("1", "1", "1", "1", "1", "1")
  cal <- data.frame(Name, Value, stringsAsFactors = FALSE)
  ...
  infoDf <- data.frame(infoCal)
  
  func <-
    try(NEONprocIS.cal::def.cal.func.poly(infoCal = infoDf,
                                          log = log,
                                          Vrbs = TRUE),
        silent = TRUE)
  #
  ...
  #
  cat("\n       |======= Negative test 2::                      =======================|\n")
  cat("\n       |------- infoDf is a data frame, which is not expected as an input param|\n\n")
  #
  cat("\n       |------- polynomial calibration function will not run!                  |\n")
  cat(
    "\n       |======================================================================|\n"
