##############################################################################################
#' @title Unit test of NEON uncertainty quantification (def.ucrt.meas.mult)
...
test_that("testing uncertainty quantification (def.ucrt.meas.mult", {
  ...
  uncertainty <-
    NEONprocIS.cal::def.ucrt.meas.mult(data = data, varUcrt='data', calSlct=calSlct)

  # Check the data inside the valid date range are uncertainty correctly
  testthat::expect_equal(c(2,3,4,5)*0.00585, uncertainty$data$ucrtMeas[2:5])
  ...
  testthat::expect_equal(c(1*0.00585, 6*0.006), c(uncertainty$data$ucrtMeas[1],uncertainty$data$ucrtMeas[6]))
  ...
  uncertainty <- NEONprocIS.cal::def.ucrt.meas.mult(data = data, varUcrt='data', calSlct=calSlct)
  ...
  uncertainty <- NEONprocIS.cal::def.ucrt.meas.mult(data = data, 
                                                  varUcrt='data', 
  ...
  uncertainty <- try(NEONprocIS.cal::def.ucrt.meas.mult(data = data, varUcrt='data2', calSlct=calSlct), silent = TRUE)
  ...
  uncertainty <- try(NEONprocIS.cal::def.ucrt.meas.mult(data = data, varUcrt='data', calSlct=calSlct), silent = TRUE)
  ...
  uncertainty <- try(NEONprocIS.cal::def.ucrt.meas.mult(data = data[,-1], varUcrt='data', calSlct=calSlct), silent = TRUE)
  ...
  uncertainty <- try(NEONprocIS.cal::def.ucrt.meas.mult (data = data, 
                                                       varUcrt='data', 
                                                       calSlct=calSlct),
                    silent=TRUE)
