...
test_that("Unit test of def.ucrt.wq.do.conc.R", {
  ...
  uncertainty <-
    NEONprocIS.cal::def.ucrt.wq.do.conc(data = data, varUcrt='data', calSlct=calSlct)
  
  col_List = c('ucrtPercent','ucrtMeas')  
  
  # Check the data inside the valid date range are uncertainty correctly
  testthat::expect_true(all(uncertainty$data$ucrtPercent==0.01))
  testthat::expect_true(all(uncertainty$data$ucrtMeas==0.01*data$data))
  ...
  uncertainty <- try(NEONprocIS.cal::def.ucrt.wq.do.conc(data = data, varUcrt='data2', calSlct=calSlct), silent = TRUE)
  ...
  uncertainty <- try(NEONprocIS.cal::def.ucrt.wq.do.conc(data = data, varUcrt='data3', calSlct=calSlct), silent = TRUE)
