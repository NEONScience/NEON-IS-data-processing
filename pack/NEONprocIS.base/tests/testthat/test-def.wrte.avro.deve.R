...
test_that("When only required parameters are sent to the def.wrte.avro.deve function ",
          {
            ...
            ravroLib <- file.path(workingDirPath,"ravro.so")
            schm <- file.path(workingDirPath,"tests/testthat/def.wrte.parq/prt_calibrated.avsc")
            outputData <- NEONprocIS.base::def.wrte.avro.deve(data=data, NameFile = NameFile, NameLib= ravroLib)
...
#             workingDirPath <- getwd()
#             NameFile <- file.path(workingDirPath,"out.avro")
#             Schm <- file.path(workingDirPath,"tests/testthat/def.schm.avro.pars/prt_calibrated.avsc")
