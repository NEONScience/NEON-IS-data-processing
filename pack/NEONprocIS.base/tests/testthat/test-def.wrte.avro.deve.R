...
test_that("   Testing def.wrte.avro.deve.R, definition function. Write AVRO file",
          {
            ...
            if (os_type == "Linux")
            {
              ...
              # testDir = "testdata/"
              # testFile = "HARV_data.csv"
              # testFilePath <- paste0(testDir, testFile)
              # 
              # data <- as.data.frame(read.csv(testFilePath, ,sep=",",row.names=NULL,header=T))
              
              nameFile <- file.path(workingDirPath, "testdata/HART_data.avro")
              nameLib <- file.path(workingDirPath, "ravro.so")
              data_df <- def.read.avro.deve(NameFile = nameFile, NameLib = nameLib) 
            ...
            # schm <- file.path(workingDirPath, "tests/testthat/testdata/HART_data.avsc")
            # writeSuccess <- NEONprocIS.base::def.wrte.avro.deve(data = data,NameFile = outFile,NameSchm = Schm,NameLib = 'ravro.so')
