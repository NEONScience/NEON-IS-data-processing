...
# pfsDir <- "/scratch/pfs"
pfsDir <- "/home/NEON/jroberti/pfs"#location_active_dates/"
...
latestPipelineFiles=listPachFiles1(repo = "pqs1_merge_data_by_location", subDir = "pqs1/2019/01/01/*/*")
...
getPachFiles(repo = "pqs1_merge_data_by_location", pachFiles = filesNeeded, somDirectory = paste0(pfsDir,"/empty_files/"))


# Loop thru the files we pulled down to som
for(file in dir(paste0(pfsDir,"/empty_files/pqs1/"), recursive=T, full.names=T)){
  ...
  emptyDir=paste0(pfsDir,"/empty_files/pqs1/", newDir)
  
  # Make the directory, if needed
  if(!dir.exists(paths = emptyDir)){
    dir.create(paste0(pfsDir,"/empty_files/pqs1/", newDir))
  }
  ...
  if(grepl(pattern = "parquet", x = file)){
    ...
    NEONprocIS.base::def.wrte.parq(data = emptyData, NameFile = paste0(emptyDir, "/pqs1_location_year-month-day", appendName, ".parquet"))
  }else if(grepl(pattern = "json", x = file)){
    writeLines(text="{}", con =  paste0(emptyDir, "/pqs1_location_year-month-day", appendName, ".json"))
  }
}

# clean out our bad file structures, before moving
system(paste0("rm -R ",pfsDir,"/empty_files/pqs1/2019/"))

# move our new files over. Currently this moves all of the stuff in
putPachFiles(repo="empty_files", pachDir = "pqs1", somDirectory = paste0(pfsDir,"/empty_files/pqs1"))

# OPTIONAL - Check out if it went OK
listPachFiles1(repo = "empty_files", subDir = "/**")
