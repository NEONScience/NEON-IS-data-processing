...
listPachFiles1=function(repo, subDir){
  library(magrittr)
...
listPachFiles2=function(repo, subDir){
  ...
  foundFiles=system(command = paste0('pachctl list file ', repo, '@', subDir), intern = T) %>%
    ...
    gsub(pattern = " file [0-9.]*[aA-zZ]{1,} ", replacement = "") %>%
    trimws()
  #get index of first white space.  If it exsits, truncate anything past it"
  truncEnd<-gregexpr(pattern =' ',foundFiles[1])[[1]][1]
  if(length(truncEnd)!=0){
    foundFiles<-sapply(foundFiles, function(x) substr(x = x,1,truncEnd-1))
  }
  if(length(foundFiles)>0){
    return(foundFiles)
  }else{
...
getPachFiles=function(repo, pachFiles, somDirectory,commitID="master"){
  # Loop thru files
  for(i in 1:length(pachFiles)){
    getCmd=paste0('pachctl get file -r ', repo, '@',commitID,':', pachFiles[i],' -o ', paste0(somDirectory, "/", pachFiles[i]))
    #replace any instances of forward slashes:
    gsub("/{2}|/{3}","/",getCmd)
