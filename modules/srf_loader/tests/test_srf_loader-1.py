...
class SrfLoaderTest(DatabaseBackedTest):
    ...
    def test_srf_loader(self):
        ...
        def get_srfs(group_prefix)  -> Iterator[Srf]:
            srfs.append(Srf(group_name=group_name,id=id1,start_date=to_datetime(s_date),
            ...
            print(f'[srfs] in test_group_loader before return: {[srfs]}')
            srfs.append(srfs)
            print(f'[srfs] before return: {[srfs]}')
            return srfs
