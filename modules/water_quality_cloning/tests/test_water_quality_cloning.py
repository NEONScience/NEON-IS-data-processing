...
class WaterQualityCloningTest(unittest.TestCase):
    ...
    def test_get_named_location(self):
        with closing(cx_Oracle.connect(self.db_url)) as connection:
            ...
            named_location = named_location_creator.get_named_location(156303)
    ...
    def test_get_new_name(self):
        new_name = NamedLocationCreator.get_clone_name(1000)
        self.assertTrue(new_name == 'SENSOR001000')
        new_name = NamedLocationCreator.get_clone_name(999)
        self.assertTrue(new_name == 'SENSOR000999')
        new_name = NamedLocationCreator.get_clone_name(100)
        self.assertTrue(new_name == 'SENSOR000100')
        new_name = NamedLocationCreator.get_clone_name(99)
        self.assertTrue(new_name == 'SENSOR000099')
        new_name = NamedLocationCreator.get_clone_name(10)
        self.assertTrue(new_name == 'SENSOR000010')
        new_name = NamedLocationCreator.get_clone_name(9)
        self.assertTrue(new_name == 'SENSOR000009')

    def test_clone(self):
        with closing(cx_Oracle.connect(self.db_url)) as connection:
            named_location_creator = NamedLocationCreator(connection)
            source_named_location = named_location_creator.get_named_location(156303)
    ...
    def test_save_clone(self):
        ...
        with closing(cx_Oracle.connect(self.db_url)) as connection:
            # create the clone
            named_location_creator = NamedLocationCreator(connection)
            source_named_location = named_location_creator.get_named_location(source_key)
            ...
            # save the clone
            clone_key = named_location_creator.save_clone(cloned_named_location)
            # check the clone
            clone_from_db = named_location_creator.get_named_location(clone_key)
            ...
            with closing(connection.cursor()) as delete_cursor:
                ...
                connection.commit()
            # check clone deleted
            deleted_clone = named_location_creator.get_named_location(clone_key)
            self.assertTrue(deleted_clone is None)

    def test_asset_assigner_sensor_type_index(self):
        index = AssetAssigner.get_clone_location_index('exo2')
        self.assertTrue(index == 0)

    def test_get_field_names(self):
        field_names = AssetAssigner.get_field_names('exofdom')
    ...
    def test_location_tree_assign_parent(self):
        ...
        with closing(cx_Oracle.connect(self.db_url)) as connection:
            ...
            parent_id = location_tree_assigner.get_parent(named_location_id)
            # create entry
            location_tree_assigner.assign_to_same_parent(named_location_id, child_id)
            # get entry
            entry = location_tree_assigner.get_location_tree(parent_id, child_id)
            ...
            with closing(connection.cursor()) as delete_cursor:
                ...
                connection.commit()
            deleted_entry = location_tree_assigner.get_location_tree(parent_id, child_id)
