...
class ArrayParserTest(TestCase):
    ...
    def setUp(self) -> None:
        ...
        target_path = Path(
            self.in_path, self.metadata_path, "data", "tchain_32610_2019-01-12.parquet"
        )
        self.fs.add_real_file(actual_path, target_path=target_path)

        # calibrations
        actual_path = Path(
            os.path.dirname(__file__), "30000000016555_WO12477_87280.xml"
        )
        target_path = Path(
            self.in_path,
            self.calibration_metadata_path,
            "30000000016555_WO12477_87280.xml",
        )
        self.fs.add_real_file(actual_path, target_path=target_path)

        actual_path = Path(
            os.path.dirname(__file__), "30000000016555_WO12477_87330.xml"
        )
        target_path = Path(
            self.in_path,
            self.calibration_metadata_path,
            "30000000016555_WO12477_87330.xml",
        )
        self.fs.add_real_file(actual_path, target_path=target_path)

        actual_path = Path(
            os.path.dirname(__file__), "30000000016555_WO12477_87331.xml"
        )
        target_path = Path(
            self.in_path,
            self.calibration_metadata_path,
            "30000000016555_WO12477_87331.xml",
        )
        self.fs.add_real_file(actual_path, target_path=target_path)

        actual_path = Path(
            os.path.dirname(__file__), "30000000016555_WO12477_87332.xml"
        )
        target_path = Path(
            self.in_path,
            self.calibration_metadata_path,
            "30000000016555_WO12477_87332.xml",
        )
        self.fs.add_real_file(actual_path, target_path=target_path)

        actual_path = Path(
            os.path.dirname(__file__), "30000000016555_WO12477_87333.xml"
        )
        target_path = Path(
            self.in_path,
            self.calibration_metadata_path,
            "30000000016555_WO12477_87333.xml",
        )
        self.fs.add_real_file(actual_path, target_path=target_path)

        actual_path = Path(
            os.path.dirname(__file__), "30000000016555_WO12477_87334.xml"
        )
        target_path = Path(
            self.in_path,
            self.calibration_metadata_path,
            "30000000016555_WO12477_87334.xml",
        )
        self.fs.add_real_file(actual_path, target_path=target_path)

        actual_path = Path(
            os.path.dirname(__file__), "30000000016555_WO12477_87335.xml"
        )
        target_path = Path(
            self.in_path,
            self.calibration_metadata_path,
            "30000000016555_WO12477_87335.xml",
        )
        self.fs.add_real_file(actual_path, target_path=target_path)

        actual_path = Path(
            os.path.dirname(__file__), "30000000016555_WO12477_87336.xml"
        )
        target_path = Path(
            self.in_path,
            self.calibration_metadata_path,
            "30000000016555_WO12477_87336.xml",
        )
        self.fs.add_real_file(actual_path, target_path=target_path)

        actual_path = Path(
            os.path.dirname(__file__), "30000000016555_WO12477_87337.xml"
        )
        target_path = Path(
            self.in_path,
            self.calibration_metadata_path,
            "30000000016555_WO12477_87337.xml",
        )
        self.fs.add_real_file(actual_path, target_path=target_path)

        actual_path = Path(
            os.path.dirname(__file__), "30000000016555_WO12477_87338.xml"
        )
        target_path = Path(
            self.in_path,
            self.calibration_metadata_path,
            "30000000016555_WO12477_87338.xml",
        )
        self.fs.add_real_file(actual_path, target_path=target_path)
    ...
    def test_main(self) -> None:
        ...
        os.environ["USING_FILESYSTEM"] = str(True)
    ...
    def check_output(self) -> None:
        ...
        calibration_path_0 = Path(
            calibration_root_path, "depth0WaterTemp", "30000000016555_WO12477_87280.xml"
        )
        calibration_path_1 = Path(
            calibration_root_path, "depth1WaterTemp", "30000000016555_WO12477_87330.xml"
        )
        calibration_path_2 = Path(
            calibration_root_path, "depth2WaterTemp", "30000000016555_WO12477_87331.xml"
        )
        calibration_path_3 = Path(
            calibration_root_path, "depth3WaterTemp", "30000000016555_WO12477_87332.xml"
        )
        calibration_path_4 = Path(
            calibration_root_path, "depth4WaterTemp", "30000000016555_WO12477_87333.xml"
        )
        calibration_path_5 = Path(
            calibration_root_path, "depth5WaterTemp", "30000000016555_WO12477_87334.xml"
        )
        calibration_path_6 = Path(
            calibration_root_path, "depth6WaterTemp", "30000000016555_WO12477_87335.xml"
        )
        calibration_path_7 = Path(
            calibration_root_path, "depth7WaterTemp", "30000000016555_WO12477_87336.xml"
        )
        calibration_path_8 = Path(
            calibration_root_path, "depth8WaterTemp", "30000000016555_WO12477_87337.xml"
        )
        calibration_path_9 = Path(
            calibration_root_path, "depth9WaterTemp", "30000000016555_WO12477_87338.xml"
        )
