...
class PadderUtilTest(unittest.TestCase):
    ...
    def test_getDatesInPaddedRange(self):
        ...
        self.assertEqual(pad_calculator.get_dates_in_padded_range(datetime.date(2018, 6, 15), 1), date_range)
        date_range = [datetime.date(2018, 6, 14), datetime.date(2018, 6, 15), datetime.date(2018, 6, 16)]
        self.assertEqual(pad_calculator.get_dates_in_padded_range(datetime.date(2018, 6, 15), 0.1), date_range)
        ...
        self.assertEqual(pad_calculator.get_dates_in_padded_range(datetime.date(2017, 12, 31), 2), date_range)
        date_range = [datetime.date(2018, 6, 14)]
        self.assertEqual(pad_calculator.get_dates_in_padded_range(datetime.date(2018, 6, 14), 0), date_range)
        # test -1 for only preceding day
        expected_date_range = [datetime.date(2018, 6, 14), datetime.date(2018, 6, 15)]
        date_range = pad_calculator.get_dates_in_padded_range(datetime.date(2018, 6, 15), -1)
